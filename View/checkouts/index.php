    ...
    if(!empty($_POST)) {
        ...
        foreach($data as $item) {
            ...
            if($address != null) {
                ...
                ('$user_name','$user_name','$phone_number','$email','$address','$created_at',$product_id,'$num',0)";
                execute($sql);
                $sql = "Delete from cart where user_name = '$user_name'";
                execute($sql);
                $_SESSION['user_tam'] = $user_name;
                header("Location: index.php?action=success&act=1");
            } else {
            ...
            foreach($data as $item) {
                ...
                if($address != null) {
                    ...
                    execute($sql);
                    $sql = "Delete from cart where user_name = 'Client'";
                    execute($sql);
