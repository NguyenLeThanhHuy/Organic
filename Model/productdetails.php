<?php 
    class productdetails{
        ...
        function insertsp($name,$price,$img,$type,$category_id,$description,$slt,$slug)
        {      
            $db=new connect();      
            $query="insert into product_details (mahh,category_id,name,type,price,img,description,slug,soluongton)
             values(null ,$category_id, '$name','$type','$price', '$img','$description','$slug',$slt)";
            $db->exec($query);    
        }
        ...
        function getThongKeMatHang($nam = null , $thang = null , $ngay = null) {
            $db = new connect();
            if(!empty($nam) && !empty($thang) && !empty($ngay)) {
                $select = "SELECT a.created_at,b.name,sum(a.num) as soluong FROM hoadon a, product_details b WHERE a.product_id=b.mahh and day(a.created_at) = $ngay and month(a.created_at)= $thang and year(a.created_at)= $nam GROUP BY b.name,a.created_at;";
                            
            }
            else {
                $select = "SELECT b.name,sum(a.num) as soluong FROM hoadon a, product_details b WHERE a.product_id=b.mahh GROUP BY b.name";
            }
