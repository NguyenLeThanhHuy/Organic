...
class user
{
    ...
    {
        $db = new connect();
        $select = "SELECT * FROM user where makh = $id";
        $result = $db->getInstance($select);
        return $result;
    }
    ...
    function deletecomment($namehh, $isCheck = false)
    {   
        $db = new connect();
        if($isCheck) {
            $query = "delete from binhluan1 where namekh = '$namehh'";
        }else {
    ...
    {
        $db = new connect();
        $query = "delete from user where makh = $id";
        $db->exec($query);
    }
